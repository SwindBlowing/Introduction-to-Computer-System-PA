...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, if (dest) R(dest) = *get_csr(imm); *get_csr(imm) = src1;printf("%x %x\n", s->pc, cpu.mtvec););
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(dest) = *get_csr(imm); if (BITS(s->isa.inst.val, 19, 15)) *get_csr(imm) |= src1;printf("%x %x\n", s->pc, cpu.mtvec););
