...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (imm + src1) & 0xFFFFFFFEu, R(dest) = s->snpc);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2 & 0x0000FFFFu));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, !(imm & 0x00000010) ? R(dest) = (int)src1 >> (imm & 0x0000001Fu) : 0);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2 & 0x000000FFu));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, ((int)src1 >= (int)src2) ? s->dnpc = imm + s->pc : 0);
