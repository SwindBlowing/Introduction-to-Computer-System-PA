...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, printf("%x ", cpu.mtvec); if (dest) R(dest) = *get_csr(imm); *get_csr(imm) = src1;printf("%x %x\n", s->pc, cpu.mtvec););
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, printf("%x ", cpu.mtvec); R(dest) = *get_csr(imm); if (BITS(s->isa.inst.val, 19, 15)) *get_csr(imm) |= src1;printf("%x %x\n", s->pc, cpu.mtvec););
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = cpu.mtvec; printf("!!!%x\n", cpu.mtvec));
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(0x0000000b, s->pc); printf("after ecall%x\n", cpu.mtvec));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv    , N, INV(s->pc));
  INSTPAT_END();

  if (s->pc == 0x8000060c) printf("imm%x\n", imm);
