...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, (src1 == src2) ? s->dnpc = imm + s->pc : 1);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, (src1 != src2) ? s->dnpc = imm + s->pc : 1);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, ((int)src1 >= (int)src2) ? s->dnpc = imm + s->pc : 1);
